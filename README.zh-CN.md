...
这是一个面向所有 AI agents 的 COMSOL 项目护栏型 skill，用于帮助 agent 在处理 COMSOL Multiphysics 项目时减少常见错误。
...
`agents/openai.yaml` 是给 Codex/OpenAI 风格 skill 界面使用的可选展示配置。其他 agent 可以忽略这个文件。

## 在 Codex 中使用

将整个文件夹放到 Codex skills 目录中：
...
如果使用其他 AI agent 系统，可以直接复制或引用 `SKILL.md` 作为操作指南。这个 skill 是平台中立的，重点是 COMSOL 工作流、验证、交付物追踪和风险规避，而不是某一个特定 agent 平台。
